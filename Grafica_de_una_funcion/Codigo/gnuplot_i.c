...
void tablaC(int r1, int r2, int m, int b){
	...
	int x[r],y[r]; //asignacion del rango para la cantidad de datos que van a contener
	
	for (i=r1;i<r2;i++){//se asigan los valores de las coordenadas en "x" y "y"
        x[j]=i;
        y[j]=(m*i)+b;
        j++;
    }
    printf("\nLas coordenadas son:\a");
    printf("\n\t---------------------------------");
    printf("\n\t|\tx\t|\ty=mx+b\t|");
    printf("\n\t---------------------------------");
    for (i=0;i<r;i++){
...
/**
*@brief    graficar() da inicio a la graficacion de la funcion.
*\details  En esta seccion se utilizan las funciones que ya se encontraban ene sta libreria con anterioridad.
*
...
*@param     r1       Para alojar el rango inferior o izquierdo. 
*@param     r2       Para alojar el rango superior o derecho.
*@param     t        Permite al usuario elegir cuanto tiempo puede ver la grafica.
*@return   void
*/
void graficar(int r1, int r2,int m, int b, int t){
	
	gnuplot_ctrl * inicioS;	//iniciar Gnuplot, inicio sesion
    inicioS = gnuplot_init();
    
    gnuplot_setstyle(inicioS, "lines"); //aspecto
    gnuplot_cmd(inicioS,"set xzeroaxis lt -1");//coloca el eje x
	gnuplot_cmd(inicioS,"set yzeroaxis lt -1");//coloca el eje y
    gnuplot_set_ylabel(inicioS, "Eje X");//nombre de las etiquetas en y
    gnuplot_set_xlabel(inicioS, "Eje Y");//nombre de las etiquetas en x
    gnuplot_cmd(inicioS,"set xrange [%d:%d]",r1, r2);//define el rango x
    gnuplot_cmd(inicioS,"set yrange [%d:%d]",r1,r2 );//define el rango y
    gnuplot_plot_slope(inicioS,m,b, "--Funcion de la forma y=mx+b--"); //crea el grafico con la funcion
    
    sleep(t);
	
	gnuplot_close (inicioS);//cerrar la sesion para elliminar posibles archivos residuales
